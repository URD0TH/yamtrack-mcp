...
async function handleServeStatus() {
    await pm2Connect();
    const list = await pm2List();
    await pm2Disconnect();
    const proc = list.find((p) => p.name === "yamtrack-mcp");
    if (!proc) {
...
async function handleServeLogs() {
    await pm2Connect();
...
async function handleServeRestart() {
    await pm2Connect();
...
async function handleServeSave() {
    await pm2Connect();
    await pm2Dump();
    await pm2Disconnect();
