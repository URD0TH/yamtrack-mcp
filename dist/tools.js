...
const LIST_STATUS = ["All", ...STATUS_VALUES];
const HOME_SORT = [
    "upcoming",
    "recent",
    "completion",
...
const PROGRESS_OP = ["increase", "decrease"];
const SOURCES = [
    "tmdb",
    "mal",
    "igdb",
...
function summarizeStatistics(raw) {
    const sd = raw.status_distribution;
    const statusSummary = {};
    if (sd?.datasets && Array.isArray(sd.datasets)) {
        for (const ds of sd.datasets) {
            statusSummary[ds.label] = ds.total;
        }
    }
    const avg = raw.score_distribution?.average_score;
    const consumption = (raw.consumption_stats ?? []).map(({ color: _c, ...item }) => item);
    return {
...
function summarizeSearchResults(raw) {
...
function summarizeDetails(raw) {
    ...
    const seasons = (meta.related?.seasons ?? []);
    const recs = (meta.related?.recommendations ?? []);
    const links = meta.external_links ?? {};
    return {
        ...metaRest,
...
function register(server, name, description, schema, fn, client) {
...
export function registerTools(server, client) {
    // --- Search & Browse ---
    register(server, "search_media", "Search external providers for media by title.", {
        ...
        return summarizeSearchResults(raw);
    }, client);
    register(server, "get_details", "Get metadata for a media item from a provider.", {
        ...
        if (a.media_type === "tv" && a.season_number !== undefined) {
            raw = await c.request("GET", `/details/${a.source}/tv/${a.media_id}/season/${a.season_number}/`);
        }
        else {
            raw = await c.request("GET", `/details/${a.source}/${a.media_type}/${a.media_id}/`);
        }
        return summarizeDetails(raw);
    }, client);
    // --- Tracked Media ---
    register(server, "list_tracked_media", "List the authenticated user's tracked media.", {
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
        status: z
            .enum(LIST_STATUS)
            .optional()
        ...
        sort: z
            .string()
            .optional()
            .describe("Sort field: score, title, progress, start_date, end_date, or any item field."),
        ...
        return summarizeMediaList(raw);
    }, client);
    register(server, "get_home", "Dashboard with in-progress and planning items.", {
        sort: z
            .enum(HOME_SORT)
            .optional()
    ...
    register(server, "get_history", "Change history for a tracked media item.", {
        source: z.enum(SOURCES).describe("Provider source."),
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
        media_id: z.string().describe("Provider media id."),
    ...
    // --- Actions ---
    register(server, "create_entry", "Start tracking media from an external provider by id.", {
        media_id: z.string().describe("Provider media id."),
        source: z.enum(SOURCES).describe("Provider source."),
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
        ...
        if (a.progress !== undefined)
            body.progress = a.progress;
        if (a.notes !== undefined)
            body.notes = a.notes;
        if (a.start_date !== undefined)
            body.start_date = a.start_date;
        if (a.end_date !== undefined)
            body.end_date = a.end_date;
        return c.request("POST", `/media/${a.media_type}/create/`, { body });
    }, client);
    register(server, "manual_create", "Create a media entry manually (no external provider).", {
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
    ...
    register(server, "update_entry", "Update a tracked media item (status, score, progress, notes).", {
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
    ...
    register(server, "update_progress", "Increase or decrease progress on a tracked item.", {
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
        instance_id: z.coerce.number().int().describe("Tracked item instance id."),
    ...
    register(server, "update_score", "Update the score (0-10) of a tracked item.", {
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
        instance_id: z.coerce.number().int().describe("Tracked item instance id."),
    ...
    register(server, "delete_entry", "Delete a tracked media item.", {
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
        instance_id: z.coerce.number().int().describe("Tracked item instance id."),
    }, async (c, a) => {
        return c.request("DELETE", `/media/${a.media_type}/${a.instance_id}/delete/`);
    }, client);
    register(server, "sync_metadata", "Re-sync metadata for a media item from its provider.", {
        source: z.enum(SOURCES).describe("Provider source."),
        media_type: z.enum(MEDIA_TYPES).describe("Type of media."),
        media_id: z.string().describe("Provider media id."),
    }, async (c, a) => {
    ...
    register(server, "create_episode", "Mark an episode as watched.", {
        media_id: z.string().describe("Provider media id."),
        source: z.enum(SOURCES).describe("Provider source."),
    ...
    register(server, "get_statistics", "Aggregated statistics for the authenticated user.", {
        ...
        return summarizeStatistics(raw);
    }, client);
    register(server, "get_me", "Get the currently authenticated user.", {}, async (c) => {
        const raw = (await c.request("GET", "/auth/me/"));
